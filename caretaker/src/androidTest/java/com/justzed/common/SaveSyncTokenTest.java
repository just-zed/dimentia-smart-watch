package com.justzed.common;
import android.test.ActivityTestCase;
import android.test.ActivityUnitTestCase;
import android.test.suitebuilder.annotation.SmallTest;

import com.justzed.caretaker.MainActivity;
import com.justzed.common.model.PatientLink;
import com.justzed.common.model.Person;

import junit.framework.TestCase;

import org.junit.Assert;
import org.junit.Before;
import org.junit.Test;

/**
 * Created by Shirin on 8/23/2015.
 */
public class SaveSyncTokenTest extends ActivityUnitTestCase<MainActivity> {
    public SaveSyncTokenTest() {
        super(MainActivity.class);
    }

   // @Before
    //public void setUpTests(){
        //SaveSyncToken syncToken = new SaveSyncToken(Activity);

    //}
    /*@Test
    public void testfindDeviceIdNotNull() {
        String careTakerToken = saveSyncToken.findDeviceId();
        assertNotNull(careTakerToken);
    }*/

    //public String patientDeviceID(){
    //String PatientDeviceID =
    //return "";
   /*public void testGetPatientDeviceIdException()
    {
        //syncToken.getPatientDeviceId
    }*/
}



